...
apply(time1, 2, mean)["elapsed"]
```
without parallelization.  But we did not get a 4-fold speedup with 4 cores.
There is a cost to sending information to and from the slave processes.
And some
time needs to be taken from this number crunching to run the rest of the
computer.  However, we did get a 2-fold speedup.  If we had
more slaves, we could do even better.
