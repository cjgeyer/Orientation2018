 ...
 * R function `mle` calculates the estimator by calling R function `nlm` to
   minimize it.  The starting value, either `sign(mean(x)) * sd(x)` is a
...
```{r engine='bash', eval=FALSE}
qsub -I -q multinode -l nodes=3:ppn=4
cd tmp/Orientation2018 # or wherever
...
The `qsub` command says we want to use 4 cores on each of 3 nodes (computers)
for 12 cores total.
